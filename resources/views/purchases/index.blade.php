...
<tr><td>{{ $order->order_number }}</td><td>{{ $order->supplier->name }}</td><td>{{ $order->order_date->format('M d, Y') }}</td><td>${{ number_format($order->total_amount,2) }}</td><td>{{ ucfirst($order->status) }}</td><td class="text-end"><div class="table-actions"><a href="{{ route('purchases.show',$order) }}" class="btn btn-sm btn-outline-info"><i class="fa-solid fa-eye"></i></a></div></td></tr>
@empty<tr><td colspan="6" class="text-center py-4 text-muted">No purchase orders yet.</td></tr>@endforelse
</tbody></table></div>@include('components.table-pagination', ['paginator'=>$orders, 'sorts'=>['created_at'=>'Created','order_number'=>'PO #','status'=>'Status','total_amount'=>'Total']])</div>
@endsection
