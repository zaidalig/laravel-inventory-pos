...
<div class="col-md-6"><label class="form-label">Image</label>@if($product->image_path)<div class="mb-2"><img src="{{ media_url($product->image_path) }}" alt="{{ $product->name }}" style="width:64px;height:64px;object-fit:cover;border-radius:8px;"></div>@endif<input type="file" name="image" class="form-control" accept=".jpg,.jpeg,.png,.webp"><div class="form-text">Leave empty to keep the current image.</div>@error('image')<div class="text-danger small mt-1">{{ $message }}</div>@enderror</div>
