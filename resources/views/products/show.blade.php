...
@section('content')
<div class="row g-4">
<div class="col-lg-4"><div class="card border-0 shadow-sm p-4">@if($product->image_path)<img src="{{ media_url($product->image_path) }}" alt="{{ $product->name }}" class="img-fluid rounded mb-3" style="max-height:240px;width:100%;object-fit:cover;">@endif<h4 class="fw-bold">{{ $product->name }}</h4><p class="text-muted">SKU: {{ $product->sku }}</p><p>Category: {{ $product->category?->name ?? '-' }}</p><p>Supplier: {{ $product->supplier?->name ?? '-' }}</p><p>Stock: <strong>{{ $product->stock_qty }} {{ $product->unit }}</strong></p><p>Cost: ${{ number_format($product->cost_price,2) }} | Price: ${{ number_format($product->sale_price,2) }}</p><a href="{{ route('products.edit',$product) }}" class="btn btn-sm btn-outline-primary">Edit</a></div></div>
